...
static inline uint32_t ovs_refcount_read(uint32_t *refcnt) {
	if (refcnt != NULL) {
		return *refcnt;
	}
...
static inline void ovs_refcount_ref(uint32_t *refcnt) {
	if (refcnt != NULL) {
		(*refcnt) ++;
	}
}
 ...
 * in this form:
 */
static inline uint32_t ovs_refcount_unref(uint32_t *refcnt) {
	if (refcnt == NULL) {
		return 0;
	}
